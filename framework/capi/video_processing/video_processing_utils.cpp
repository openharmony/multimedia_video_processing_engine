...
const std::unordered_map<VideoProcessing_ErrorCode, std::string> CAPI_ERROR_STR_MAP = {
...
const std::unordered_map<VideoProcessing_State, std::string> CAPI_STATE_STR_MAP = {
...
VideoProcessing_ErrorCode VideoProcessingUtils::InnerErrorToCAPI(VPEAlgoErrCode errorCode)
{
...
VideoProcessing_State VideoProcessingUtils::InnerStateToCAPI(VPEAlgoState state)
{
...
{
    auto it = CAPI_STATE_STR_MAP.find(state);
    if (it == CAPI_STATE_STR_MAP.end()) [[unlikely]] {
        VPE_LOGE("Invalid state:%{public}d", state);
